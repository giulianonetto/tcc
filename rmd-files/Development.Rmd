...
```{r Figure2, fig.cap = "Figure 2 - Gene modules distribution.", fig.height = 8, fig.width = 15, fig.align = "center"}
...
```{r Figure3, fig.cap = "Figure 3 - Example of successfully normalized microarray data."}

data_path <- "data/ExampleMicroarrayData/expressionData.tsv"
ExpData <- read.table(data_path, sep = "\t", 
                      header = T, row.names = 1, stringsAsFactors = F)

longExpData <- gather(ExpData[which(ExpData < 15),1:8])
p1 <- ggplot(longExpData, aes(x = value, color = key))+
      geom_density(show.legend = FALSE)+
      ggtitle("Densities")+
      xlab("Intensity")+ylab("Frequency")
p2 <- ggplot(longExpData, aes(x = key, y = value))+
      geom_boxplot(show.legend = FALSE)+
      ggtitle("Quartiles")+
      xlab("Arrays")+ylab("Intensiy")
...
## Detection and removal of batch effects

  __TO-DO__ see: [@Leek2012; Li2007; Benito2004]
