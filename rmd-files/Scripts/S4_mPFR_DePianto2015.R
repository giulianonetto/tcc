  ...
  # Build eSet objct
{
  ...
  # YOU CAN START FROM HERE - LOAD PROCESSED DATA
{
  ...
  # Compute PCAs - with outliers
{
  ...
  loadings.df <- data.frame(Load = PCA.all$sdev, Eigen = 1:length(PCA.all$sdev))
  pp1 <- autoplot(PCA.all, x = 1, y = 2, 
                  data = pheno, colour = "Source", 
                  shape = "Treatment", size = 3)
  pp2 <- ggplot(loadings.df,
                aes(y = Load, x = Eigen)) + geom_point()
  ...
  # Compute PCAs - without outliers
{
  ...
  loadings.df <- data.frame(Load = PCA.clean$sdev, Eigen = 1:length(PCA.clean$sdev))
  pp1 <- autoplot(PCA.clean, x = 1, y = 2, 
           data = pheno[!outliers.vec,], colour = "Source", 
           shape = "Treatment", size = 3)
  pp2 <- ggplot(loadings.df,
                  aes(y = Load, x = Eigen)) + geom_point()
  plots <- ggarrange(pp1, pp2, ncol = 2)
  annotate_figure(plots, 
                  top = text_grob("PCA - IPF patients - without outliers\n",
                                  face = "bold", size = 20, 
                                  vjust = 1))
}
